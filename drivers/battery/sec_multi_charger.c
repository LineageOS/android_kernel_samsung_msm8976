...
static int sec_multi_chg_check_charging_current(struct sec_multi_charger_info *charger)
{
	...
	if (charger->sub_is_charging) {
		...
		{
			unsigned int diff = sub_charging_current - charger->pdata->sub_charger_current_max;
			sub_charging_current = charger->pdata->sub_charger_current_max;
			main_charging_current = main_charging_current + diff;
		}
			    ...
			    union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		...
		pr_info("%s: main input current(%dmA)\n", __func__,
			val->intval);
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		/* AICL Enable */
		if(!charger->pdata->aicl_irq_disable)
			psy_do_property(charger->pdata->main_charger_name, set, psp, value);
		break;
	default:
		return -EINVAL;
		...
		struct sec_multi_charger_info *charger)
{
	...
	if (!np) {
		...
		pdata->is_serial = of_property_read_bool(np,
			"charger,is_serial");
		pdata->aicl_irq_disable = of_property_read_bool(np,
			"charger,aicl_irq_disable");
		...
		if (pdata->sub_charger_condition) {
			...
			if (ret) {
				...
				pdata->sub_charger_enable_current_now = 1400;
			}
		...
		unsigned long action, void *data)
{
	...
	struct sec_multi_charger_info *charger =
		container_of(nb, struct sec_multi_charger_info,
			     charger_nb);
	struct charger_notifier_struct * charger_noti = (struct charger_notifier_struct *)data;

	pr_info("%s: call notification\n", __func__);
	pr_info("%s: action : %ld\n", __func__, action);
	pr_info("%s: main_charger name : %s\n", __func__,charger->pdata->main_charger_name);
	pr_info("%s: current : %d\n", __func__,charger_noti->input_current);
	return 0;
}
#endif
...
#if defined(CONFIG_BATTERY_NOTIFIER)
	charger_notifier_register(&charger->charger_nb,
