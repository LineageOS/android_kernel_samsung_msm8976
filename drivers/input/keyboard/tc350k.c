...
*/
	...
	printk(KERN_DEBUG "[TK] %s: tkey_scl= %d, tkey_sda= %d, tkey_int= %d, tkey_sub_det= %d, en=%d, f/w=%s%s",
...
int tc300k_touchkey_power(struct tc300k_data *data, bool on)
{
	...
	if (tc300k_power_enabled == on){
		printk(KERN_DEBUG "[TK] %s - already on\n", __func__);
		return 0;

	}

	gpio_direction_output(data->pdata->en_gpio, on);
