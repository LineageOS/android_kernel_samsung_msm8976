...
static int msm8952_rt5659_start_sysclk(struct snd_soc_card *card)
{
	...
	ret = clk_set_rate(pdata->rt5659_ext_clk, (unsigned long)24000000);
	if (ret < 0) {
		pr_err("%s: Can't set ext-mclk rate 24MHz (%d)\n",
		__func__, ret);
		return ret;
	}
	...
	if (ret < 0) {
		pr_err("%s: Can't enable ext-mclk %d\n",
		__func__, ret);
		return ret;
	}
